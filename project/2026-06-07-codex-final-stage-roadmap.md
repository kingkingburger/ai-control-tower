...
- 2단계 워크플로 레이어는 강하다. `ai-control-tower`, `harness/`, `.omc`,
...
- 5단계 자산 동기화는 재료가 많지만 미완이다. Codex skills, Claude skills,
  `.omc`, `harness/`, 프로젝트별 문서가 공존하고 있으며 단일 원본과 배포 규칙이
  아직 명확하지 않다.
...
- `harness/git-hooks/octoto/pre-commit`
- `harness/git-hooks/octoto/commit-msg`

위 hook은 `harness/bin/octoto-harness.ps1`를 호출하지만, 현재 체크아웃에는 해당
파일이 없다. 따라서 현재 상태를 "훅 기반 자동화 완료"로 보면 안 된다. 먼저
깨진 런타임 경로를 복구하거나 hook 자체를 정리해야 한다.

또 하나의 갭은 자산 원본의 분산이다.

- Codex 전역 스킬: `C:\Users\dnjsa\.codex\skills`
- 개인 하네스 원본 후보: `harness/`
...
- 공통 작업 규칙은 `harness/core/`에서만 수정한다.
- 프로젝트별 private overlay는 `harness/projects/<project>/`에서만 수정한다.
...
- `C:\Users\dnjsa\.codex\skills`, `harness/`, `.claude/skills`, `.omc`를 목록화한다.
- 각 자산을 `원본`, `배포본`, `폐기 후보`, `검토 필요`로 분류한다.
- `harness/bin/octoto-harness.ps1`를 복구할지, 기존 hook을 제거할지 결정한다.
- `harness/git-hooks/octoto/*`가 실제로 실행 가능한지 검증한다.

완료 기준:

- 깨진 hook 호출이 없다.
- 자산 인벤토리 문서가 존재한다.
- `harness/` 안의 절대경로 사용처가 의도된 것인지 확인되어 있다.
...
- 공통 규칙 원본을 `harness/core/`로 고정한다.
- 프로젝트별 private overlay 원본을 `harness/projects/<project>/`로 고정한다.
- Codex용 skill 원본을 `harness/skills/` 또는 별도 `harness/distribution/codex/`
  중 하나로 확정한다.
- Claude, OMC, 프로젝트 문서는 배포 대상이라는 규칙을 문서화한다.

완료 기준:

- `harness/README.md`만 읽어도 원본과 배포본의 차이를 알 수 있다.
...
- 공유 프로젝트 문서에는 협업자에게 안전한 규칙만 둔다.
- 개인 선호와 반복 학습은 `ai-control-tower`의 private overlay에 둔다.
...
1. 자산 인벤토리 문서를 만든다.
2. `harness/bin/octoto-harness.ps1` 누락을 복구 또는 제거 방향으로 결정한다.
3. `harness/README.md`에 원본/배포본 경계를 더 명확히 쓴다.
