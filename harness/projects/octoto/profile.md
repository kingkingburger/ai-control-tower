...
`octoto`는 다른 사람과 함께 보는 저장소다. 개인 선호, 세션 메모리, 실험적인
하네스 메모, 개인 전략을 이 저장소에 저장하지 않는다.
...
- 세션 종료는 커밋과 `closing-lite` 메모리 축적을 포함한다.
- Octoto 공유 문서는 깨끗하게 유지하고, 팀에 안전한 반복 규칙만 승격한다.
