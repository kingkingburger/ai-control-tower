...
<!-- ✅ Block 6 완료 (연결 방법: Connector) -->

| 항목 | 값 |
|------|-----|
| 연결 방법 | Connector (claude.ai/settings/connectors) |
| MCP 도구 | `mcp__claude_ai_Slack__slack_read_channel`, `mcp__claude_ai_Slack__slack_search_public` |
| 수집 범위 | 최근 7일 |
...
각 채널에 대해 mcp__claude_ai_Slack__slack_read_channel 호출.
  mcp__claude_ai_Slack__slack_read_channel(channel="개발소통방", limit=50)
  mcp__claude_ai_Slack__slack_read_channel(channel="team-dt1", limit=50)
나에게 멘션된 메시지는 mcp__claude_ai_Slack__slack_search_public으로 검색.
  mcp__claude_ai_Slack__slack_search_public(query="to:<@U08NHL77BS6>", sort="timestamp")
```
...
<!-- 참고: claude.ai Connector로 Notion이 이미 연결되어 있음, local에도 notion MCP 등록됨 -->
...
<!-- 참고: claude.ai Connector로 Gmail, Google Calendar가 이미 연결되어 있음 -->
